    ...
    public static void create(Context ctx) {
        try {
            String name = ctx.formParamAsClass("name", String.class)
                    .check(str -> str.length() > 1, "Название курса должно быть не короче двух символов")
                    .get();
            ...
            BuildPostPage page = new BuildPostPage(ctx.formParam("name"),
                    ctx.formParam("body"), exception.getErrors());
            ctx.render("posts/build.jte", model("page", page));
        }
    }
